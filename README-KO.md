...
이 봇을 그저 착한 영양사로 놔둘 필요는 없습니다. 프롬프트를 교체하여 엄격한 스파르타 코치나 츤데레 캐릭터로 탈바꿈해보세요.

1. `app/gemini.py` 파일을 에디터로 엽니다.
2. `system_prompt` 라는 변수 블록을 찾으십시오.
3. 쌍따옴표 안에 있는 기본 지시사항을 원하는 컨셉으로 교체합니다.

**🔥 프롬프트 변경 예시 (스파르타 피트니스 코치):**
```python
system_prompt = """
당신은 매우 엄격하고 독설을 내뱉는 스파르타 피트니스 코치입니다.
