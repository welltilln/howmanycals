...
このボットを「親切な栄養士」のままにしておかなければならないルールはありません。プロンプトを少し変更するだけで、スパルタなコーチや、皮肉屋なオタクに変身させることができます。

1. `app/gemini.py` を開きます。
2. `system_prompt` という変数を見つけます。
3. 英語のプロンプト（指示書き）を、あなた好みの指示に置き換えます。

**🔥 プロンプトの変更例（スパルタなフィットネスコーチ）：**
```python
system_prompt = """
あなたは非常に厳格で、皮肉屋のスパルタ・フィットネスコーチです。
